...
static class Menus
{
    ...
    public static string[] unrenamedMatchesArray;
}
...
public class Creator : MonoBehaviour
{
    ...
    void Start()
    {
        questionImageDataList = new List<byte[]>();
        // Initialize the rename matches array
...
/*        if(Globals.currentlyChangingFile == true)
    ...
    {
        Debug.Log("Deleting duplicate files");
...
/*        // Check if a path was selected, a question created
    ...
    {
        // Reset the rename matches array
        Menus.unrenamedMatchesArray = new string[5];
    ...
    public void RenameMatches()
    {
        ...
        if(modelNameMatch1.GetComponentInChildren<TMP_Text>().text != "")
        {
            if(enterNewModelNameMatch1.text != "")
            {
                ...
                // Save the unrenamed matches in the array
                Menus.unrenamedMatchesArray[0] = modelNameMatch1.GetComponentInChildren<TMP_Text>().text;
            }
        }
        ...
        if(modelNameMatch2.GetComponentInChildren<TMP_Text>().text != "")
        {
            if(enterNewModelNameMatch2.text != "")
            {
                ...
                // Save the unrenamed matches in the array
                Menus.unrenamedMatchesArray[1] = modelNameMatch2.GetComponentInChildren<TMP_Text>().text;
            }
        }
        ...
        if(modelNameMatch3.GetComponentInChildren<TMP_Text>().text != "")
        {
            if(enterNewModelNameMatch3.text != "")
            {
                ...
                // Save the unrenamed matches in the array
                Menus.unrenamedMatchesArray[2] = modelNameMatch3.GetComponentInChildren<TMP_Text>().text;
            }
        }
        ...
        if(modelNameMatch4.GetComponentInChildren<TMP_Text>().text != "")
        {
            if(enterNewModelNameMatch4.text != "")
            {
                ...
                // Save the unrenamed matches in the array
                Menus.unrenamedMatchesArray[3] = modelNameMatch4.GetComponentInChildren<TMP_Text>().text;
            }
        }
        ...
        if(modelNameMatch5.GetComponentInChildren<TMP_Text>().text != "")
        {
            if(enterNewModelNameMatch5.text != "")
            {
                ...
                // Save the unrenamed matches in the array
                Menus.unrenamedMatchesArray[4] = modelNameMatch5.GetComponentInChildren<TMP_Text>().text;
            }
        }
    }

    // Uncomment if we want add models later
...
/*    public void AddModelInformation()
    ...
    // Method used to merge changed models (models of a changed question)
    public void MergeChangedModels()
    {
        // Get the models array of the temp save folder
        string[] modelsTemSave = ReturnPreviewModelNames();
        ...
        string[] oldModelNames = GetModelsNamesFromArray(modelsEndSave);

        // Get the models array of the end save folder
        string[] modelFilesTempSave = ReturnModelNames(Menus.tempSavePath);

        // Check if all model files in the temp save file folder is still linked to this question
        foreach(string modelFiles in modelFilesTempSave)
        {
            // Check if the name is contained
            if(isContained(modelsTemSave, Path.GetFileName(modelFiles)) == false)
            {
                // Delete the file that contains that model name
                string modelFilePath = GetFileFromModelName(Menus.tempSavePath, Path.GetFileName(modelFiles));
        ...
        // Get the models array of the end save folder
        string[] modelFilesTempSavePostDeletion = ReturnModelNames(Menus.tempSavePath);

        // Get the array of old models names
        string[] loadedModelNames = GetModelsNamesFromArray(modelFilesTempSavePostDeletion);

        // Check if no name coincide
        foreach(string modelTemp in modelsTemSave)
        {
            if(isContained(oldModelNames, modelTemp) == true)
            {
                if(isContained(loadedModelNames, modelTemp) == false)
                {
                    // Add the name of the file that already exist in the end save directory to the array
                    modelMatches[matches] = modelTemp;
                    ...
                    // Find the button
                    Button previewButton = ReturnPreviewModelButtonFromModelName(modelTemp);

                    switch(previewButton.gameObject.name)
                    {
                        case "Add3DModel1":
                            Menus.unrenamedMatchesArray[0] = modelTemp;
                        break;

                        case "Add3DModel2":
                            Menus.unrenamedMatchesArray[1] = modelTemp;
                        break;

                        case "Add3DModel3":
                            Menus.unrenamedMatchesArray[2] = modelTemp;
                        break;

                        case "Add3DModel4":
                            Menus.unrenamedMatchesArray[3] = modelTemp;
                        break;

                        case "Add3DModel5":
                            Menus.unrenamedMatchesArray[4] = modelTemp;
                        break;
                    }
                }
            }
        ...
        if(matches == 0)
        {
            // Launch the main save method
            ChangeQuestionInEndDirectory();
    ...
    public int RenameModels(string path, int index)
    {
        ...
        Debug.Log("When renaming models, the number of models was: " + numberOfModelsInPreview());
        ...
        int numberOfModelsThatNeedNames = numberOfModelsInPreview();
        ...
        Debug.Log("The number of times model files should be created should be: " + numberOfModelsThatNeedNames);

        while(indexRest < numberOfModelsThatNeedNames)
        {
            // Generate the right index at the end of the name
            string ending = ReturnQuestionIndex(currentIndex);
            string name = "Model" + ending + ".json";

            if(oldModelNames != null)
            {
                foreach(string oldModelName in oldModelNames)
                {
                    Debug.Log("Checking if: " + name + " was equalto: " + oldModelName);
                }
            }
            ...
            if(isContained(oldModelNames, name) != true)
            {
                ...
                if(isContained(Menus.unrenamedMatchesArray, modelName) == true)
                {
                    ...
                    model.numberOfQuestionsUsedIn = model.numberOfQuestionsUsedIn + Menus.currentQuestionIndex;
                    ...
                    // Increase the number of newly added models
                    newAddedModelNumber = newAddedModelNumber + 1;
                }

                // Increase the index rest because a model file was created
                indexRest = indexRest + 1;

                // Decrease the number that still need to be created
        ...
        Debug.Log("the number of newly added models is: " + newAddedModelNumber);
        // Return the number of old models + the number of newly added models
        return index + newAddedModelNumber;
    }
    ...
    // Method that is triggered when clicking on the import image of the import 3D model window
    public async void ImportImage()
    {
        ...
        // string url = enterUrl.text;
        string endingPNG = ".png";
        string endingJPG = ".jpg";
        ...
        if(string.IsNullOrEmpty(url))
        {
            ...
            if(!url.EndsWith(endingPNG) && !url.EndsWith(endingJPG))
            {
                Debug.Log("Url is not ending on .png or .jpg, assuming incorrect file type.");
                ...
                if(File.Exists(Menus.tempSavePath + fileName))
                {
                    ...
                    Menus.imageURL = url;
                    await DownloadAndShowImageAsync(url);
                    DeactivateImportModelWindow();
    ...
    // Method that returns the array of model names in the given array
    static string[] GetModelsNamesFromArray(string[] array) 
    {
        // Initialize an array of the same length
        string[] newArray = new string[array.Length];

        // Initialize the index
        int index = 0;

        // Go through all model paths in the given array
        foreach(string model in array)
        {
            // Insert the model name
            newArray[index] = Path.GetFileName(model);
        ...
        if(newName == "")
        {
            ...
            if(isContained(array, newName) == true && Menus.editedModelName != newName)
            {
                ...
                if(Menus.editedModelIndex != 5 && Globals.currentlyChangingFile == false)
                {
                    ...
                    Debug.Log("The current model index should have been reseted to: " + array.Length);

                    // Set correctly the current model index
                    Menus.currentModelIndex = array.Length;
